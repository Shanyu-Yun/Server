...
先看最里面的`weak_ptr<TcpConnection>`。要意识到时间轮的角色仅仅是个"监督者"，连接真正的所有权在`TcpServer`、`TcpConnection`那一套体系里。<u>如果这里用的是`shared_ptr`</u>，那么时间轮反而会延长连接的寿命，甚至和连接互相持有形成<span style="color:crimson">循环引用</span>，导致谁都析构不了。所以这里用`weak_ptr`，表达的是"我只是看你还活着没有，并不掌控你的生死"。
...
不过这里又会冒出一个问题，`onMessage`是怎么快速找到"这个连接对应的那个`Entry`"的呢？答案是把`weak_ptr<Entry>`反向存进了`TcpConnection`身上，而这又依赖`TcpConnection`新增的一个上下文字段，定义如下：
...
这里用了`std::any`，它可以装任意类型，`TcpConnection`对里面到底放了什么一无所知，取用的时候再由调用方用`std::any_cast`还原回去。这其实是一种很常见的解耦手法，底层只负责留一个"随身口袋"，上层想塞什么业务数据都行，而底层不必去认识上层的`TimingWheel`。在这里口袋里塞的正是`weak_ptr<Entry>`，同样用的是`weak_ptr`而非`shared_ptr`，因为<u>连接绝不能反过来强持有`Entry`</u>，<span style="color:crimson">否则又绕回了循环引用</span>。
...
逻辑很直白，给新连接造一个`Entry`，放进最新的格子（队尾），再把对应的`weak_ptr<Entry>`塞进连接的`context_`口袋里，供之后刷新超时时取用。
...
正如前面所说，从口袋里取回`weak_ptr<Entry>`并`lock()`，再把这个`shared_ptr<Entry>`重新插进队尾的最新格，就完成了一次续命。这里用`unordered_set`做格子也是有讲究的，它<u>天然去重</u>，所以同一秒内连续收到多个数据包也不会重复插入同一个`Entry`。
...
至此，时间轮把"N秒空闲超时"这件事，从"每连接一个定时器、高频增删红黑树"压缩成了全局仅一个每秒定时器、新增与刷新都是O(1)哈希插入，而连接何时关闭则完全交给`shared_ptr<Entry>`的引用计数自动驱动，<span style="color:dodgerblue">`TcpConnection`本身零侵入</span>。使用时只需把`onConnection`挂到`TcpServer`的连接回调、`onMessage`挂到消息回调即可。回过头再看那三层包裹，其实每一层都在精确地解决一个问题，`weak_ptr<TcpConnection>`防循环引用，`Entry`把关闭绑定到析构，`shared_ptr<Entry>`用引用计数表达存活窗口，理解了这三者的配合，整个超时机制也就豁然开朗了。
