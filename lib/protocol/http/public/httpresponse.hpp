...
class HttpResponse {
 public:
   ...
   * 空行，最后是 body。
   */
  void appendToBuffer(transport::Buffer* out) const;
  ...
  /** @brief 为 true 时 appendToBuffer 不输出任何字节（WS 升级用：自行发 101）。 */
 ...
 private:
  ...
  bool streaming_  = false;    ///< 响应体是否为流式响应
  bool bypass_     = false;    ///< 为 true 时 appendToBuffer 不输出任何内容（供 WS 升级使用）
};
