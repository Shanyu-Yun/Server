...
void TcpConnection::send(const char* message, size_t len) {
  if (state_ == StateE::kConnected) {
    if (loop_->isInLoopThread())
      sendInLoop(std::string(message, len));
    else
      loop_->runInLoop([self = shared_from_this(), message, len]() { self->sendInLoop(std::string(message, len)); });
  }
}
...
void TcpConnection::sendInLoop(const std::string& message) {
  ...
  if (message.size() > highWaterMark_) {
    LOGERROR("TcpConnection::sendInLoop() [{}] message too large: {} bytes (highWaterMark {})", name_, message.size(),
             highWaterMark_);
    return;
  }
